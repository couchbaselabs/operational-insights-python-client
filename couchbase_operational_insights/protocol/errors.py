...
from couchbase_operational_insights.common.errors import (
...
class WrappedError(Exception):
    ...
    def maybe_set_cause_context(self, context: ErrorContext) -> None:
        if not isinstance(self._cause, (OperationalInsightsError, InvalidCredentialError, QueryError, TimeoutError)):
            return
...
class ErrorMapper:
    ...
    def build_error_from_http_status_code(message: str, context: ErrorContext) -> WrappedError:
        if context.status_code == 503:
            return WrappedError(OperationalInsightsError(context=str(context), message=message), retriable=True)
        if context.status_code == 404:
            return WrappedError(QueryNotFoundError(context=str(context), message=message), retriable=False)

        return WrappedError(OperationalInsightsError(context=str(context), message=message))
    ...
    def build_error_from_json(json_data: List[Dict[str, Any]], context: ErrorContext) -> WrappedError:  # noqa: C901
        if context.status_code is None:
            return WrappedError(OperationalInsightsError(context=str(context), message='Unknown error occurred.'))
        ...
        if first_err is None:
            ...
            return WrappedError(OperationalInsightsError(context=str(context), message=err_msg))
    ...
    def maybe_get_error_from_status_code(
        ...
        if status_code == 401:
            ...
            err = WrappedError(OperationalInsightsError(context=context, message='Internal server error (HTTP 500).'))
        elif status_code == 503:
            err = WrappedError(
                OperationalInsightsError(context=context, message='Service unavailable.'), retriable=True
            )
    ...
    def handle_socket_error(
        ...
        def wrapped_fn(host: str, port: int, logger_handler: Optional[Callable[..., None]] = None) -> str:
            try:
                ...
                raise WrappedError(OperationalInsightsError(cause=ex, message=msg), retriable=True) from None
